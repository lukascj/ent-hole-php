...
function make_table($conn, $name, $columns) {

    echo var_dump($columns);
    $columns_str = implode(",", $columns);
    $sql = "CREATE TABLE $name ( $columns_str );";
...
make_table($conn, $name, $columns);

function db_fetch($conn, $query, $params) {

    $stmt = mysqli_stmt_init($conn);
    if(!mysqli_stmt_prepare($stmt, $query)) {
        header("location: /?error=mysqli-stmt");
        exit;
    }
    ...
    foreach($params as $param) {
        ...
        if($char != "") {
            ...
            header("location: /?error=stmt-params");
            exit;
        }
    }
    
    if(strlen($params_chars) > 0) {
        mysqli_stmt_bind_param($stmt, $params_chars, ...$params);
    }
...
function db_edit($conn, $query, $params) {
