...
class User {
    ...
    public function fetch($handle) {
        ...
        $query = "SELECT * FROM users WHERE handle = ? LIMIT 1";
        return $this->_conn->execute_query($query, [$handle])->fetch_assoc();
    }

    public function fetch_by_id($id) {
        $query = $this->_conn->prepare("SELECT * FROM users WHERE id = ?");
        return $this->_conn->execute_query($query, [$id])->fetch_row()[0];
    }
    ...
    public function fetch_activity($id, $lim = 19) {
        ...
        $query = "SELECT 
            ...
            FROM 
                entries 
            INNER JOIN 
                ents ON ents.id = entries.ent_id 
            INNER JOIN 
                follows ON follows.to_id = entries.user_id 
            INNER JOIN 
                users ON users.id = follows.to_id 
            WHERE 
                follows.to_id IN (
                    SELECT 
                        to_id 
        ...
        return $this->_conn->execute_query($query, [$id, $lim])->fetch_all(MYSQLI_ASSOC);
    }
